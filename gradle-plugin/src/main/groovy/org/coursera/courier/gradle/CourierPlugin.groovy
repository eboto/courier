...
class CourierPlugin implements Plugin<Project> {
    ...
    project.sourceSets.all { SourceSet sourceSet ->
      ...
      if (isTestSourceSet(sourceSet)) {
        configureGeneration(new SourceSetConfig(
            ...
            sourceSet: sourceSet,
            compile: testCompile,
            dataModel: project.configurations.testDataModel,
            courier: project.configurations.testCourierDataBinding,
        ...
        configureGeneration(new SourceSetConfig(
            ...
            sourceSet: sourceSet,
            compile: compile,
            dataModel: project.configurations.dataModel,
  ...
  protected void configureGeneration(SourceSetConfig config) {
    ...
    Task generatorTask = config.project.task(config.generateDataBindingTask(), type: GeneratorTask) {
      ...
      resolverPath = config.dataModel
      onlyIf {
        config.dataSchemaDir().exists()
      }
      // TODO: delete dir before generating into it
      //doFirst {
    ...
    SourceSet targetSourceSet = config.sourceSetContainer().create(config.targetSourceSetName()) {
      ...
      compileClasspath = config.dataModel + config.courierCompile
    }

    // Intellij plugin.
    addGeneratedDir(
        config.project, targetSourceSet, [config.dataModel, config.courierCompile])

    // Make sure that code is generated before compiling it.
    final Task compileGeneratedClassesTask =
        config.project.tasks[targetSourceSet.getCompileTaskName("java")] // set to java when needed
    ...
    config.project.artifacts {
      ...
      "${config.dataModel.name}" schemaJarTask
    }
    ...
    //config.project.tasks[config.sourceSet.getCompileTaskName("scala")].dependsOn(dataBindingJarTask)
  }
  ...
  static class GeneratorTask extends DefaultTask {
    ...
    protected void generate() {
      System.err.println("Pegasus Generator Task: ")
      System.err.println("destinationSourceSetDir: " + destinationSourceSetDir)
      System.err.println("inputDir: " + inputDir)
      System.err.println("resolverPath: " + resolverPath.asPath)
      ...
      System.err.println("resolverPathStr: " + resolverPathStr  )
